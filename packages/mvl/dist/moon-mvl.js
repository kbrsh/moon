...
(function(root, factory) {
	...
	module.exports = function (name, input) {
		var inputJS = null;
		var inputCSS = null;
		var scriptMatch = input.match(scriptRE);

		if (scriptMatch !== null) {
			inputJS = scriptMatch[1];
		}

		var styleMatch = input.match(styleRE);
		...
		if (inputCSS !== null) {
			...
			transform = function transform(tree) {
				addClass(tree, scope);
				return tree;
			};
		...
		if (inputJS !== null) {
			outputJS = "import Moon from \"moon\";" + compile(inputJS, {
				transform: transform
			});
		}
